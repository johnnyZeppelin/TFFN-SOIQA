...
The original paper is not open-sourced. This repository follows the paper structure where the details are clear and uses a practical, runnable approximation where details are missing. The PDIE branch is still using a VMamba-like placeholder encoder at this stage, not the official VMamba implementation.
